...
// uxf/limits.ts
var CAR_IMPORT_MAX_BLOCK_COUNT, CAR_IMPORT_MAX_BLOCK_BYTES, VERIFY_MAX_ELEMENT_BYTES, EXTRACT_CAR_ROOT_HEADER_PROBE_BYTES, CAR_IMPORT_MAX_TOTAL_BYTES, MANIFEST_MAX_SIZE, ELEMENTS_MAX_SIZE, MAX_CREATOR_LENGTH, MAX_DESCRIPTION_LENGTH;
...
function deconstructSmtPath(pool, merkleTreePath) {
  const sdkPath = import_SparseMerkleTreePath.SparseMerkleTreePath.fromJSON(merkleTreePath);
...
var import_dag_cbor4, import_SparseMerkleTreePath;
var init_deconstruct = __esm({
  "uxf/deconstruct.ts"() {
    "use strict";
    import_dag_cbor4 = require("@ipld/dag-cbor");
    import_SparseMerkleTreePath = require("@unicitylabs/state-transition-sdk/lib/mtree/plain/SparseMerkleTreePath.js");
    init_errors4();
  }
});
...
function assembleSmtPath(pool, pathHash, ctx) {
  ...
  const sdkPath = import_SparseMerkleTreePath2.SparseMerkleTreePath.fromCBOR(bytes);
...
var import_dag_cbor5, import_SparseMerkleTreePath2;
var init_assemble = __esm({
  "uxf/assemble.ts"() {
    "use strict";
    import_dag_cbor5 = require("@ipld/dag-cbor");
    import_SparseMerkleTreePath2 = require("@unicitylabs/state-transition-sdk/lib/mtree/plain/SparseMerkleTreePath.js");
    init_types();
    init_element_pool();
    init_instance_chain();
    init_hash();
...
__export(browser_exports, {
  createBrowserProfileProviders: () => createBrowserProfileProviders,
  createBrowserProfileProvidersFromSphere: () => createBrowserProfileProvidersFromSphere,
  migrateLegacyToProfileBrowser: () => migrateLegacyToProfileBrowser
});
  ...
  async get(key) {
    ...
    try {
      ...
      if (lostCid !== null) {
        logger.warn(
          "ProfileStorage",
          `[LOAD-BLOCK-MISSING] Local Helia blockstore is missing block ${lostCid} for key="${translated.profileKey}". Returning null so a caller with a fallback storage can attempt to satisfy the read from legacy cache. Run an OpLog auto-reset (write a fresh entry) to clear the dangling head.`
        );
        return null;
      }
      throw err;
...
// profile/browser.ts
function createBrowserProfileProviders(config) {
...
async function createBrowserProfileProvidersFromSphere(sphere, config) {
...
async function migrateLegacyToProfileBrowser(opts) {
