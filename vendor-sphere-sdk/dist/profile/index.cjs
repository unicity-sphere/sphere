...
function encodeEntry(entry) {
  ...
  const bytes = (0, import_dag_cbor.encode)(plain);
...
function decodeEntry(bytes) {
  ...
  let decoded;
  try {
    decoded = (0, import_dag_cbor.decode)(bytes);
  } catch (err) {
    throw new OpLogEntryCorrupt(
...
var import_dag_cbor, OPLOG_ENTRY_SCHEMA_VERSION, OPLOG_ENTRY_LEGACY_VERSION, MAX_ENVELOPE_BYTES, MAX_PAYLOAD_BYTES, KNOWN_ENVELOPE_FIELDS, MIN_PLAUSIBLE_TS, OpLogEntryCorrupt;
...
async function putBlockToLocalHelia(helia, cidString, blockBytes) {
  let parsed;
  try {
    parsed = import_cid.CID.parse(cidString);
  } catch (err) {
    logger.warn(
      "ipfs-client",
...
async function fetchCarFromIpfs(gateways, rootCid, timeoutMs = DEFAULT_FETCH_TIMEOUT_MS2, maxSizeBytesPerBlock = DEFAULT_MAX_SIZE_BYTES, helia) {
  ...
  while (queue.length > 0) {
    ...
    let blockCid;
    try {
      blockCid = import_cid.CID.parse(cidStr);
    } catch (err) {
      throw new ProfileError(
        "BUNDLE_NOT_FOUND",
...
function collectCidLinks(value, visit) {
  ...
  const asCid = import_cid.CID.asCID(value);
  if (asCid !== null) {
    visit(asCid);
    return;
  }
...
function contentHashBytesToCid(bytes) {
  return import_cid.CID.createV1(CODEC_DAG_CBOR, (0, import_digest.create)(MULTIHASH_SHA256, bytes));
}
function walkUxfElement(node, visit) {
  ...
  if (predecessor instanceof Uint8Array && predecessor.byteLength === SHA256_DIGEST_BYTES) {
    ...
    const asCid = predecessor != null ? import_cid.CID.asCID(predecessor) : null;
...
var import_car2, import_decoder, import_buffer, MAX_DECODE_CONTENT_BYTES;
var init_transfer_payload = __esm({
  "uxf/transfer-payload.ts"() {
    "use strict";
    import_car2 = require("@ipld/car");
    import_decoder = require("@ipld/car/decoder");
...
function computeElementHash(element) {
  ...
  const cborBytes = (0, import_dag_cbor3.encode)(canonical);
  const hashBytes = sha256(cborBytes);
  return contentHash(bytesToHex6(hashBytes));
}
var import_dag_cbor3, BYTE_FIELDS;
var init_hash = __esm({
  "uxf/hash.ts"() {
    "use strict";
    init_sha2();
...
function encodeReason(reason) {
  ...
  return (0, import_dag_cbor4.encode)(reason);
}
...
var import_dag_cbor4;
var init_deconstruct = __esm({
  "uxf/deconstruct.ts"() {
    "use strict";
    import_dag_cbor4 = require("@ipld/dag-cbor");
    init_errors4();
  }
});
...
function decodeReason(reason) {
  ...
  try {
    return (0, import_dag_cbor5.decode)(reason);
  } catch {
...
function verify(pkg) {
  ...
  for (const [hash, element] of pkg.pool) {
    ...
    try {
      const probe = (0, import_dag_cbor6.encode)({
...
var import_dag_cbor6, EXPECTED_CHILD_TYPES, EXPECTED_ARRAY_CHILD_TYPES;
...
function contentHashToCid(hash) {
  ...
  return import_multiformats.CID.createV1(DAG_CBOR_CODE2, digest);
}
...
function elementToIpldBlock(element) {
  ...
  const bytes = (0, import_dag_cbor7.encode)(canonical);
  const hashBytes = sha256Sync(bytes);
  const digest = createSha256Digest(hashBytes);
  const cid = import_multiformats.CID.createV1(DAG_CBOR_CODE2, digest);
  return { cid, bytes };
}
async function exportToCar(pkg) {
  ...
  const manifestBytes = (0, import_dag_cbor7.encode)(manifestNode);
  const manifestHashBytes = sha256Sync(manifestBytes);
  const manifestDigest = createSha256Digest(manifestHashBytes);
  const manifestCid = import_multiformats.CID.createV1(DAG_CBOR_CODE2, manifestDigest);
  const envelopeNode = {
  ...
  const envelopeBytes = (0, import_dag_cbor7.encode)(envelopeNode);
  const envelopeHashBytes = sha256Sync(envelopeBytes);
  const envelopeDigest = createSha256Digest(envelopeHashBytes);
  const envelopeCid = import_multiformats.CID.createV1(DAG_CBOR_CODE2, envelopeDigest);
  const { writer, out } = import_writer2.CarWriter.create([envelopeCid]);
  const chunks = [];
  const collectPromise = (async () => {
    for await (const chunk of out) {
      chunks.push(chunk);
...
async function importFromCar(car) {
  ...
  const reader = await import_car3.CarReader.fromBytes(car);
  ...
  const envelopeNode = (0, import_dag_cbor7.decode)(envelopeBlock.bytes);
  ...
  const manifestNode = (0, import_dag_cbor7.decode)(manifestBlock.bytes);
  ...
  for await (const block of reader.blocks()) {
    ...
    const node = (0, import_dag_cbor7.decode)(block.bytes);
...
function decodeIpldContentArray(type, key, value) {
  if (type === "smt-path" && key === "segments") {
    return value.map((seg) => {
      ...
      return {
        data: s.data instanceof Uint8Array ? bytesToHex7(s.data) : s.data,
        path: pathStr
      };
    });
  }
...
var import_dag_cbor7, import_multiformats, import_writer2, import_car3, DAG_CBOR_CODE2, TYPE_ID_TO_TAG2;
var init_ipld = __esm({
  "uxf/ipld.ts"() {
    "use strict";
    import_dag_cbor7 = require("@ipld/dag-cbor");
    import_multiformats = require("multiformats");
    init_sha2();
    import_writer2 = require("@ipld/car/writer");
    import_car3 = require("@ipld/car");
    init_types();
    init_token_join();
    init_errors4();
    init_header_validation();
...
function walkFileInner(rootCid, blocks, depth, outBuf, outBytesSoFar, pathVisited, blockBytes, visitCounter) {
  ...
  if (fs.blocksizes.length !== node.Links.length) {
    throw new Error(
      `unixfs-verify: blocksizes length (${fs.blocksizes.length}) disagrees with PBNode.Links length (${node.Links.length}) at ${rootCid.toString()}`
    );
  }
...
async function verifyCarAndExtractFile(carBytes, expectedCid) {
  const reader = await import_car4.CarReader.fromBytes(carBytes);
  const roots = await reader.getRoots();
  const root0 = roots[0];
  if (!root0) {
...
var import_car4, UNIXFS_TYPE_RAW, UNIXFS_TYPE_FILE, CODEC_RAW2, CODEC_DAGPB, MULTIHASH_SHA2562, MAX_RECURSION_DEPTH, MAX_TOTAL_OUTPUT_BYTES, MAX_LINKS_PER_NODE, MAX_TOTAL_NODE_VISITS;
var init_unixfs_verify = __esm({
  "profile/migration/unixfs-verify.ts"() {
    "use strict";
    import_car4 = require("@ipld/car");
    init_src();
    init_sha2();
    UNIXFS_TYPE_RAW = 0;
    UNIXFS_TYPE_FILE = 2;
...
__export(profile_exports, {
  CACHE_ONLY_KEYS: () => CACHE_ONLY_KEYS,
  ...
  ProfileStorageProvider: () => ProfileStorageProvider,
  ProfileTokenStorageProvider: () => ProfileTokenStorageProvider,
  auditKeyFor: () => auditKeyFor,
  computeAddressId: () => computeAddressId,
  conflictingTokenIds: () => conflictingTokenIds,
  createProfileProviders: () => createProfileProviders,
  decryptProfileValue: () => decryptProfileValue,
  decryptString: () => decryptString,
  deriveHistoryFromArchived: () => deriveHistoryFromArchived,
  deriveProfileEncryptionKey: () => deriveProfileEncryptionKey,
  deriveSentFromArchived: () => deriveSentFromArchived,
  deriveStructuralManifest: () => deriveStructuralManifest,
  deriveTombstonesFromArchived: () => deriveTombstonesFromArchived,
  encryptProfileValue: () => encryptProfileValue,
  encryptString: () => encryptString,
  fetchCarFromIpfs: () => fetchCarFromIpfs,
  fetchFromIpfs: () => fetchFromIpfs,
  importLegacyTokens: () => importLegacyTokens,
  invalidKeyFor: () => invalidKeyFor,
  isConflictingStatus: () => isConflictingStatus,
  mergeAuditEntry: () => mergeAuditEntry,
  mergeManifestEntry: () => mergeManifestEntry,
  pinCarBlocksToIpfs: () => pinCarBlocksToIpfs,
  pinToIpfs: () => pinToIpfs,
  verifyCidAccessible: () => verifyCidAccessible
});
...
async function derivePublicKeyShort(privateKeyHex) {
  ...
  try {
    ...
    const hash = hashModule.sha256(hexToBytes2(privateKeyHex));
...
// profile/nostr-replication.ts
init_encryption();
init_hex();
var NOSTR_KIND_ORBITDB_OPLOG = 29e3;
var OPLOG_TAG = "orbitdb-oplog";
var CONNECT_TIMEOUT_MS = 1e4;
var EOSE_TIMEOUT_MS = 15e3;
var MAX_EVENTS_PER_FETCH = 1e4;
var MAX_MESSAGE_LEN = 256 * 1024;
var NostrReplicationBridge = class _NostrReplicationBridge {
  constructor(config) {
    this.config = config;
  }
  // Steelman²⁸: replay-protection cache. Bounded LRU-ish set of event
  // ids we have already accepted in this session.
  static SEEN_EVENT_CACHE_MAX = 1e4;
  ...
  async publishEntry(entryData) {
    ...
    const contentHex = bytesToHex3(encrypted);
   ...
   */
  async fetchMissedEntries(since) {
    this.ensureRunning();
    ...
    return new Promise((resolve, reject) => {
      ...
      const onMessage = (msg) => {
        ...
        const raw2 = typeof data === "string" ? data : String(data);
        if (raw2.length > MAX_MESSAGE_LEN) return;
        let parsed;
        try {
        ...
        if (parsed[0] === "EVENT" && parsed[1] === subId) {
          ...
          if (totalParsedEvents >= TOTAL_EVENT_DISPATCH_CAP) {
            ...
            const truncErr = new Error(
              `Nostr fetchMissedEntries dispatch cap exceeded (${TOTAL_EVENT_DISPATCH_CAP} EVENT frames parsed). Subscription closed. Caller MUST NOT advance \`since\` \u2014 re-fetch missed window from a different relay.`
            );
            truncErr.reason = "dispatch_cap";
            truncErr.droppedCount = -1;
          ...
          if (events.length >= MAX_EVENTS_PER_FETCH) {
            ...
            cleanup();
            const truncErr = new Error(
              `Nostr fetchMissedEntries cap reached (${MAX_EVENTS_PER_FETCH} matching events). Subscription closed. Caller MUST NOT advance \`since\` \u2014 re-fetch missed window with a tighter time slice.`
            );
  ...
  connectWebSocket() {
    return new Promise((resolve, reject) => {
      ...
      if (!url.toLowerCase().startsWith("wss://") && process.env.SPHERE_ALLOW_INSECURE_RELAY !== "1") {
        reject(new Error(
          `Nostr relay URL must use wss:// (got "${url}"). Set SPHERE_ALLOW_INSECURE_RELAY=1 to allow plaintext ws://.`
        ));
        return;
      }
      const factory = this.config.createWebSocket ?? ((url2) => new WebSocket(url2));
   ...
   */
  async verifyAndDecrypt(events) {
    const VERIFY_BATCH_SIZE = 64;
    const VERIFY_TOTAL_BUDGET_MS = 5e3;
    const startedAt = Date.now();
    const authentic = [];
    let droppedCount = 0;
    ...
    if (droppedCount > 0) {
      const err = new Error(
        `Nostr verifyAndDecrypt budget exhausted (${VERIFY_TOTAL_BUDGET_MS}ms): dropped ${droppedCount} events; verified ${authentic.length}. Caller MUST NOT advance \`since\` \u2014 re-fetch missed window.`
      );
      err.droppedCount = droppedCount;
      err.authenticCount = authentic.length;
      throw err;
    }
    const replayFiltered = [];
    for (const evt of authentic) {
      if (this.#seenEventIds.has(evt.id)) continue;
      this.#seenEventIds.add(evt.id);
      replayFiltered.push(evt);
    }
    if (this.#seenEventIds.size > _NostrReplicationBridge.SEEN_EVENT_CACHE_MAX) {
  ...
  async decryptEvents(events) {
    ...
    for (const event of sorted) {
      try {
        const encrypted = hexToBytesAllowEmpty(event.content);
   ...
   */
  async buildSignedEvent(unsigned) {
    ...
    const hash = sha2563(new TextEncoder().encode(serialized));
    const id = bytesToHex3(hash);
    const secp256k1Module = await Promise.resolve().then(() => (init_secp256k1(), secp256k1_exports));
    const idBytes = hexToBytesAllowEmpty(id);
    const privKeyBytes = hexToBytesAllowEmpty(this.config.transportPrivateKey);
    let sigBytes;
    try {
    ...
    const sig = bytesToHex3(sigBytes);
    return {
      id,
   ...
   */
  async verifyEventAuthentic(evt) {
    try {
      const serialized = JSON.stringify([
        0,
        evt.pubkey,
      ...
      const { sha256: sha2563 } = await Promise.resolve().then(() => (init_sha2(), sha2_exports));
      const expectedId = bytesToHex3(sha2563(new TextEncoder().encode(serialized)));
      if (expectedId !== evt.id) return false;
      const secp256k1Module = await Promise.resolve().then(() => (init_secp256k1(), secp256k1_exports));
      const sigBytes = hexToBytesAllowEmpty(evt.sig);
      const idBytes = hexToBytesAllowEmpty(evt.id);
      const pubKeyBytes = hexToBytesAllowEmpty(evt.pubkey);
...
async function derivePublicKeyHex(privateKeyHex) {
  const secp256k1Module = await Promise.resolve().then(() => (init_secp256k1(), secp256k1_exports));
  const privKeyBytes = hexToBytesAllowEmpty(privateKeyHex);
  const pubBytes = secp256k1Module.schnorr.getPublicKey(privKeyBytes);
...
function randomSubscriptionId() {
  ...
  globalThis.crypto.getRandomValues(bytes);
  return "orbitdb_" + bytesToHex3(bytes);
}
...
var InMemoryDispositionStorageAdapter = class {
...
var import_SigningService = require("@unicitylabs/state-transition-sdk/lib/sign/SigningService.js");
async function buildPointerSigner(signingSeed) {
  const seed = signingSeed.reveal();
  try {
    const service = await import_SigningService.SigningService.createFromSecret(seed);
    const signingPubKey = service.publicKey;
    const signingPubKeyHex = bytesToHex4(signingPubKey);
...
async function writeMarker(store, v, cidBytes) {
  ...
  const rec = {
    v,
    cidHash: bytesToHex5(computeCidHash(cidBytes))
  };
...
var import_Authenticator = require("@unicitylabs/state-transition-sdk/lib/api/Authenticator.js");
var import_RequestId = require("@unicitylabs/state-transition-sdk/lib/api/RequestId.js");
var import_SubmitCommitmentResponse = require("@unicitylabs/state-transition-sdk/lib/api/SubmitCommitmentResponse.js");
var import_DataHash = require("@unicitylabs/state-transition-sdk/lib/hash/DataHash.js");
var import_HashAlgorithm = require("@unicitylabs/state-transition-sdk/lib/hash/HashAlgorithm.js");
init_errors2();
function xor32(a, b) {
...
function classifySideResult(result) {
  if (result.status === "fulfilled") {
    ...
    switch (response.status) {
      case import_SubmitCommitmentResponse.SubmitCommitmentStatus.SUCCESS:
        return { type: "success" };
      case import_SubmitCommitmentResponse.SubmitCommitmentStatus.REQUEST_ID_EXISTS:
        return { type: "exists" };
      case import_SubmitCommitmentResponse.SubmitCommitmentStatus.AUTHENTICATOR_VERIFICATION_FAILED:
        return { type: "rejected", reason: "AUTHENTICATOR_VERIFICATION_FAILED" };
      case import_SubmitCommitmentResponse.SubmitCommitmentStatus.REQUEST_ID_MISMATCH:
...
async function submitPointer(input) {
  ...
  try {
    const transactionHashA = new import_DataHash.DataHash(import_HashAlgorithm.HashAlgorithm.SHA256, ctA);
    const transactionHashB = new import_DataHash.DataHash(import_HashAlgorithm.HashAlgorithm.SHA256, ctB);
    const stateHashA = new import_DataHash.DataHash(import_HashAlgorithm.HashAlgorithm.SHA256, stateHashDigestA);
    const stateHashB = new import_DataHash.DataHash(import_HashAlgorithm.HashAlgorithm.SHA256, stateHashDigestB);
    const [authenticatorA, authenticatorB] = await Promise.all([
      import_Authenticator.Authenticator.create(signer.service, transactionHashA, stateHashA),
      import_Authenticator.Authenticator.create(signer.service, transactionHashB, stateHashB)
    ]);
    const [requestIdA, requestIdB] = await Promise.all([
      import_RequestId.RequestId.createFromImprint(signer.signingPubKey, stateHashA.imprint),
      import_RequestId.RequestId.createFromImprint(signer.signingPubKey, stateHashB.imprint)
    ]);
...
var import_RequestId2 = require("@unicitylabs/state-transition-sdk/lib/api/RequestId.js");
var import_DataHash2 = require("@unicitylabs/state-transition-sdk/lib/hash/DataHash.js");
var import_HashAlgorithm2 = require("@unicitylabs/state-transition-sdk/lib/hash/HashAlgorithm.js");
var import_InclusionProof = require("@unicitylabs/state-transition-sdk/lib/transaction/InclusionProof.js");
init_errors2();
...
async function buildRequestIds(keyMaterial, signer, v) {
  ...
  const stateHashDigestB = deriveStateHashDigest(keyMaterial.xorSeed, SIDE_B_NUM, v);
  try {
    const stateHashA = new import_DataHash2.DataHash(import_HashAlgorithm2.HashAlgorithm.SHA256, stateHashDigestA);
    const stateHashB = new import_DataHash2.DataHash(import_HashAlgorithm2.HashAlgorithm.SHA256, stateHashDigestB);
    const [reqA, reqB] = await Promise.all([
      import_RequestId2.RequestId.createFromImprint(signer.signingPubKey, stateHashA.imprint),
      import_RequestId2.RequestId.createFromImprint(signer.signingPubKey, stateHashB.imprint)
    ]);
...
async function probeVersion(input) {
  ...
  if (statusA === import_InclusionProof.InclusionProofVerificationStatus.NOT_AUTHENTICATED || statusA === import_InclusionProof.InclusionProofVerificationStatus.PATH_INVALID) {
    raiseForTrustBaseMismatch(trustBase, proofA, `probeVersion(v=${v}, side=A)`);
  }
  if (statusB === import_InclusionProof.InclusionProofVerificationStatus.NOT_AUTHENTICATED || statusB === import_InclusionProof.InclusionProofVerificationStatus.PATH_INVALID) {
    raiseForTrustBaseMismatch(trustBase, proofB, `probeVersion(v=${v}, side=B)`);
  }
  const aIncluded = statusA === import_InclusionProof.InclusionProofVerificationStatus.OK;
  const bIncluded = statusB === import_InclusionProof.InclusionProofVerificationStatus.OK;
...
async function runDecodePhases(v, keyMaterial, signer, aggregatorClient, trustBase, decodeCid, timeoutMs, abortSignal) {
  ...
  if (statusA !== import_InclusionProof.InclusionProofVerificationStatus.OK || statusB !== import_InclusionProof.InclusionProofVerificationStatus.OK) {
    return { ok: "semantic" };
  }
...
async function isReachable(input) {
  ...
  try {
    ...
    const healthCheckRequestId = import_RequestId2.RequestId.fromJSON(hex);
...
var import_DataHasher = require("@unicitylabs/state-transition-sdk/lib/hash/DataHasher.js");
var import_HashAlgorithm3 = require("@unicitylabs/state-transition-sdk/lib/hash/HashAlgorithm.js");
var import_Signature = require("@unicitylabs/state-transition-sdk/lib/sign/Signature.js");
var import_SigningService2 = require("@unicitylabs/state-transition-sdk/lib/sign/SigningService.js");
...
async function buildWinBroadcastHash(payload) {
  ...
  return new import_DataHasher.DataHasher(import_HashAlgorithm3.HashAlgorithm.SHA256).update(buf).digest();
}
...
function buildEntryGroupBlocks(entries) {
  ...
  for (const groupKey of sortedGroupKeys) {
    ...
    const groupBytes = (0, import_dag_cbor2.encode)({
      groupKey,
...
async function assembleCarBytes(snapshot, maxSizeBytes) {
  ...
  const rootBytes = (0, import_dag_cbor2.encode)({
  ...
  const { writer, out } = import_writer.CarWriter.create([rootCid]);
  const chunks = [];
  const collectPromise = (async () => {
...
async function parseLeanProfileSnapshotFromRootBlock(rootBlockBytes, fetcher) {
  ...
  let decoded;
  try {
    decoded = (0, import_dag_cbor2.decode)(rootBlockBytes);
  } catch (err) {
    throw new ProfileError(
      "PROFILE_NOT_INITIALIZED",
...
async function fetchAndDecodeAllGroupEntries(groups, fetcher) {
  ...
  const groupResults = await Promise.all(
    groups.map(async (group) => {
      ...
      let groupDecoded;
      try {
        groupDecoded = (0, import_dag_cbor2.decode)(blockBytes);
      } catch (err) {
        throw new ProfileError(
          "PROFILE_NOT_INITIALIZED",
...
var ProfileStorageProvider = class _ProfileStorageProvider {
    ...
    if (this.dbStatus !== "attached" && this.dbStatus !== "attaching" && this.dbStatus !== "fatal" && identityAtStart !== null && orbitDbConfig !== null) {
      ...
      try {
        ...
        if (pkHex && pkHex.length > 0) {
          const pkBytes = hexToBytes2(pkHex);
          try {
            const secp256k1Module = await Promise.resolve().then(() => (init_secp256k1(), secp256k1_exports));
  ...
  setIdentity(identity) {
    ...
    if (this.encryptionEnabled) {
      const privKeyBytes = hexToBytes2(identity.privateKey);
...
function isConflictingStatus(s) {
  return s === "conflicting" || s === "pending-conflicting";
}
function conflictingTokenIds(manifest) {
  const out = [];
...
init_errors();
init_encryption();
var MIGRATION_PHASE_KEY = "migration.phase";
var MIGRATION_STARTED_AT_KEY = "migration.startedAt";
var IPFS_SEQ_KEY_PATTERN = /^(?:sphere_)?ipfs_seq_/;
var IPFS_CID_KEY_PATTERN = /^(?:sphere_)?ipfs_cid_/;
var TXF_OPERATIONAL_KEYS = /* @__PURE__ */ new Set([
  "_meta",
  "_tombstones",
  "_outbox",
  "_sent",
...
var DEFAULT_IPFS_API_URL2 = "https://ipfs.unicity.network";
var ProfileMigration = class _ProfileMigration {
  ...
  constructor() {
    if (!_ProfileMigration._deprecationWarned) {
      _ProfileMigration._deprecationWarned = true;
      console.warn(
        "[ProfileMigration] DEPRECATED: this 6-step destructive migration merges remote IPFS data without cryptographic verification. New code should use importLegacyTokens (profile/import-from-legacy) which routes through the UXF integrity verifier."
      );
    }
  }
  // ---------------------------------------------------------------------------
   ...
   */
  async needsMigration(legacyStorage) {
   ...
   */
  async getMigrationPhase(legacyStorage) {
   ...
   */
  async migrate(legacyStorage, legacyTokenStorage, profileStorage, profileTokenStorage) {
    const startTime = Date.now();
    const existingPhase = await this.getMigrationPhase(legacyStorage);
    const resumeFromIndex = existingPhase !== null ? PHASE_ORDER.indexOf(existingPhase) + 1 : 0;
    ...
    try {
      ...
      return {
        ...
        durationMs: Date.now() - startTime,
        error: errorMsg,
        failedAtPhase: currentPhase ?? void 0
      };
    }
  }
  // ---------------------------------------------------------------------------
   ...
   */
  async stepSyncOldIpfs(legacyStorage, legacyTokenStorage) {
    ...
    try {
      ...
      this.log(
        "IPNS resolution failed -- migrating from local data only. Remote IPFS data may not be included."
      );
    }
  }
  // ---------------------------------------------------------------------------
  ...
  async stepTransformLocal(legacyStorage, legacyTokenStorage) {
    ...
    const accountingAndSwapKeys = /* @__PURE__ */ new Set();
    ...
    for (const rawKey of allKeys) {
      ...
      if (profileKey.includes(".accounting.") || profileKey.includes(".swap.") || profileKey.includes(".swap:")) {
    ...
    if (loadResult.success && loadResult.data) {
      ...
      const existingHistory = txfData._history ?? [];
   ...
   */
  async stepPersistToOrbitDb(profileStorage, profileTokenStorage, data) {
   ...
   */
  async stepSanityCheck(profileStorage, profileTokenStorage, data) {
    ...
    for (const [key] of data.profileKeys) {
      ...
      try {
        ...
        errors.push(
          `Failed to read back profile key '${key}': ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
    if (data.txfData !== null && data.tokenIds.size > 0) {
      ...
      if (!loadResult.success || !loadResult.data) {
        ...
        const loadedTokenIds = /* @__PURE__ */ new Set();
        ...
        if (loadedTokenIds.size < data.tokenIds.size) {
          errors.push(
            `Token count mismatch: expected at least ${data.tokenIds.size}, got ${loadedTokenIds.size}`
          );
        }
        for (const tokenId of data.tokenIds) {
          if (!loadedTokenIds.has(tokenId)) {
            const altId = tokenId.startsWith("_") ? tokenId.slice(1) : `_${tokenId}`;
        ...
        if (data.historyCount > 0) {
          ...
          if (bestHistoryCount < data.historyCount) {
            errors.push(
              `History count mismatch: expected ${data.historyCount}, got ${bestHistoryCount}`
            );
          }
        }
      }
    ...
    for (const key of data.accountingAndSwapKeys) {
      try {
        ...
        errors.push(
          `Failed to verify accounting/swap key '${key}': ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
    if (errors.length > 0) {
   ...
   */
  async stepCleanup(legacyStorage, legacyTokenStorage) {
    ...
    for (const key of allKeys) {
      const stripped = key.startsWith(STORAGE_PREFIX) ? key.slice(STORAGE_PREFIX.length) : key;
      if (stripped === MIGRATION_PHASE_KEY || stripped === MIGRATION_STARTED_AT_KEY) {
        continue;
      }
      try {
   ...
   */
  async unpinCid(cid) {
    const url = `${DEFAULT_IPFS_API_URL2}/api/v0/pin/rm?arg=${encodeURIComponent(cid)}`;
   ...
   */
  async setPhase(legacyStorage, phase) {
...
function mapLegacyKeyToProfileKey(stripped) {
  if (IPFS_STATE_KEYS_PATTERN.test(stripped)) {
    return null;
  }
...
function isCacheOnlyProfileKey(profileKey) {
  for (const [legacyKey, entry] of Object.entries(PROFILE_KEY_MAPPING)) {
...
function extractNametagTokens(txfData, tokenIds) {
  const data = txfData;
  const nametagEntry = data["_nametag"];
  if (nametagEntry !== void 0 && nametagEntry !== null && typeof nametagEntry === "object") {
    const nt = nametagEntry;
    if (nt.token !== void 0 && nt.token !== null && typeof nt.token === "object") {
  ...
  if (Array.isArray(nametagsEntry)) {
    for (let i = 0; i < nametagsEntry.length; i++) {
      const entry = nametagsEntry[i];
      if (entry !== null && entry !== void 0 && typeof entry === "object" && entry.token) {
        tokenIds.add(`_nametags_${i}`);
      }
    }
  }
}
function convertSentToHistory(sent) {
  return sent.map((entry) => ({
    dedupKey: `SENT_${entry.tokenId}_${entry.txHash}`,
    id: entry.txHash,
    type: "SENT",
    amount: "0",
    // amount not stored in legacy _sent entries
    coinId: "",
    symbol: "",
...
function mergeHistoryEntries(existing, additional) {
...
// profile/import-from-legacy.ts
init_logger();
async function importLegacyTokens(legacyTokenStorage, targetPayments, options = {}) {
  const startTime = Date.now();
...
function extractTxfTokensFromStorageData(data) {
  const tokens = [];
  let forksSkipped = 0;
  for (const [key, value] of Object.entries(data)) {
...
var ManifestCasConcurrentModificationError = class extends Error {
...
function compareCidV1Binary(a, b) {
  const aBytes = import_multiformats2.CID.parse(a).bytes;
  const bBytes = import_multiformats2.CID.parse(b).bytes;
...
// profile/disposition-writer.ts
init_errors3();
var MAX_AUDIT_BUNDLE_CIDS = 32;
var CANONICAL_CONTENT_HASH_RE = /^[0-9a-f]{64}$/i;
