 ...
 */
declare function deriveStructuralManifest(pkg: UxfPackage): TokenManifest;
/**
 ...
 */
declare function isConflictingStatus(s: TokenManifestStatus): boolean;
/**
 ...
 */
declare function conflictingTokenIds(manifest: TokenManifest): string[];
 ...
 *   manifest entry.
 */
type AuditStatus = 'audit-not-our-state' | 'audit-off-record-spend' | 'audit-promoted';
/**
 ...
 */
type ManifestEntryDelta = Pick<ManifestEntry, 'rootHash' | 'status' | 'conflictingHeads' | 'invalidReason' | 'splitParent'>;
/**
 ...
 */
type ManifestEntry = TokenManifestEntry;
 ...
 * trivially.
 */
interface MinimalManifestStorage {
    /**
 ...
 * the error escape.
 */
declare class ManifestCasConcurrentModificationError extends Error {
    readonly __manifestCasConflict: true;
...
declare class ManifestCas {
     ...
     */
    update(addr: string, tokenId: string, prev: {
        readonly contentHash: ContentHash;
    } | null, next: TokenManifestEntry): Promise<ManifestCasResult>;
}
 ...
 */
declare const CAS_MAX_RETRIES = 3;
/**
 ...
 */
declare function mergeManifestEntry(prev: TokenManifestEntry | undefined, next: TokenManifestEntry, compareCidsBinary?: (a: string, b: string) => -1 | 0 | 1): TokenManifestEntry;
/**
...
declare class ManifestStore {
     ...
     */
    upsert(addr: string, tokenId: string, next: TokenManifestEntry): Promise<TokenManifestEntry>;
    /**
     ...
     */
    addAuditPromotedFrom(addr: string, tokenId: string, auditKeys: ReadonlyArray<string>, baseDelta: TokenManifestEntry): Promise<TokenManifestEntry>;
}
 ...
 */
type DispositionEventEmitter = <T extends SphereEventType>(event: T, payload: SphereEventMap[T]) => void;
/**
 * Construction options for {@link DispositionWriter}.
 */
interface DispositionWriterOptions {
 ...
 */
declare function invalidKeyFor(addr: string, tokenId: string, observedTokenContentHash: ContentHash): string;
/**
 ...
 * hash collides.
 */
declare function auditKeyFor(addr: string, tokenId: string, observedTokenContentHash: ContentHash): string;
/**
 ...
 */
declare class DispositionWriter {
    private readonly storage;
     ...
     */
    promoteAuditEntry(addr: string, tokenId: string, observedTokenContentHash: ContentHash, manifestEntry: TokenManifestEntry): Promise<void>;
    private writeManifest;
    private writeConflictingManifest;
    private writeInvalid;
    private writeAudit;
    /**
 ...
 */
declare function mergeAuditEntry(existing: AuditEntry | undefined, incoming: Extract<DispositionRecord, {
    disposition: 'AUDIT';
}>, now: number): AuditEntry;
 ...
 */
declare const MAX_LOCK_HOLD_MS = 5000;
/** Mutex strategy selector. */
type PerTokenMutexStrategy = 'cas' | 'rpc-release' | 'bounded-hold';
interface PerTokenMutexOptions {
...
declare class PerTokenMutex {
     ...
     *   timeout case).
     */
    acquire<T>(tokenId: string, fn: () => Promise<T>, options: PerTokenMutexOptions): Promise<T>;
    /**
 ...
 */
type ProofVerifyStatus = 'OK' | 'PATH_INVALID' | 'NOT_AUTHENTICATED' | 'PATH_NOT_INCLUDED' | 'THROWN';
 ...
 */
interface ImportableInclusionProof {
    /**
 ...
 */
type ProofVerifier = (proof: ImportableInclusionProof) => Promise<ProofVerifyStatus>;
/**
 ...
 */
interface ImportProofQueueEntry {
     ...
     * removal.
     */
    readonly status: 'pending' | 'submitting' | 'polling' | 'attached' | 'hard-fail';
}
/**
 ...
 */
interface ImportProofQueueScanner {
    /**
     ...
     */
    lookupByTokenId(addr: string, tokenId: string): Promise<ReadonlyArray<ImportProofQueueEntry>>;
}
/**
 ...
 */
interface ImportProofGraftCallback {
    /**
     ...
     */
    graft(addr: string, tokenId: string, proof: ImportableInclusionProof, queueEntry: ImportProofQueueEntry): Promise<void>;
}
/**
 ...
 */
interface ImportProofOverrideCallback {
    /**
 ...
 */
type ImportProofEventEmitter = <T extends SphereEventType>(type: T, data: SphereEventMap[T]) => void;
/**
...
type ImportProofResult = {
    readonly ok: true;
    readonly transition: 'pending-still' | 'pending→valid' | 'pending→unspendable' | 'invalid→valid' | 'invalid→pending';
} | {
    readonly ok: false;
    readonly reason: 'no-such-token' | 'tokenId-already-valid' | 'tokenId-in-invalid' | 'proof-trustbase-failed' | 'proof-not-anchored' | 'requestid-mismatch' | 'proof-binding-mismatch' | 'invalid-record-missing' | 'invalid-tokenid' | 'requestid-ambiguous' | 'queue-entry-incomplete';
};
/**
 * Construction options for {@link InclusionProofImporter}.
 */
interface ImportInclusionProofOptions {
 ...
 */
interface ImportInclusionProofCallOptions {
    /**
 ...
 */
declare class InclusionProofImporter {
    private readonly opts;
    private readonly defaultNow;
    private readonly perTokenMutex;
     ...
     * can surface them.
     */
    importInclusionProof(addr: string, tokenId: string, proof: ImportableInclusionProof, callOptions?: ImportInclusionProofCallOptions): Promise<ImportProofResult>;
    /**
     * @internal
     *
     ...
     * their decisions.
     */
    private _importInclusionProofUnderMutex;
    /**
     * @internal
     *
 ...
 */
type FinalizationQueueEntryStatus = 'pending' | 'submitting' | 'polling' | 'attached' | 'hard-fail';
/**
 ...
 */
type FinalizationQueueCorruptSlotHandler = (info: {
 ...
 */
interface FinalizationQueueOptions {
 ...
 * ```
 */
declare class FinalizationQueue {
    private readonly storage;
    private readonly now;
    private readonly tombstoneRetentionMs;
    private readonly onCorruptSlot;
     ...
     * next iteration.
     */
    add(addr: string, entry: FinalizationQueueEntry): Promise<void>;
    /**
     ...
     * regression.
     */
    setPollStartedAt(addr: string, entryId: string, when: number): Promise<'set' | 'already-set' | 'absent'>;
    /**
     ...
     */
    remove(addr: string, entryId: string): Promise<void>;
    /**
     ...
     */
    hasEntry(addr: string, entryId: string): Promise<boolean>;
    /**
     ...
     */
    get(addr: string, entryId: string): Promise<FinalizationQueueEntry | undefined>;
    /**
     ...
     */
    list(addr: string): Promise<ReadonlyArray<FinalizationQueueEntry>>;
    /**
     ...
     */
    lookupByTokenId(addr: string, tokenId: string): Promise<ReadonlyArray<FinalizationQueueEntry>>;
    /**
     ...
     */
    gcTombstones(addr: string): Promise<{
 ...
 */
declare class InMemoryRecipientContextStorageAdapter {
    private readonly requestContexts;
    private readonly finalizationContexts;
    writeRequestContext(addr: string, requestId: string, record: PersistedRequestContext): Promise<void>;
    readRequestContext(addr: string, requestId: string): Promise<PersistedRequestContext | undefined>;
    deleteRequestContext(addr: string, requestId: string): Promise<void>;
    writeFinalizationContext(addr: string, tokenId: string, record: PersistedFinalizationContext): Promise<void>;
 ...
 */
type CidFetcherFetch = (input: string, init?: {
 ...
 */
type CidFetcherEmit = <K extends keyof SphereEventMap>(event: K, payload: SphereEventMap[K]) => void;
...
declare class ReplayLRU {
     ...
     */
    private readonly untrustedSenders;
    /**
 ...
 */
type AcquireBundleResult = VerifiedBundle | ReplayOutcome;
/**
 ...
 */
interface AcquireBundleCidOptions {
    /**
 ...
 */
type UxfV1Payload = UxfTransferPayloadCar | UxfTransferPayloadCid;
 ...
 */
type AcquireBundleFn = (payload: UxfV1Payload, senderPubkey: string, lru: ReplayLRU, cidOptions?: AcquireBundleCidOptions) => Promise<AcquireBundleResult>;
/**
 * Lightweight event-emit shim. The production wiring routes to the
 * Sphere event bus; tests inject a recorder so assertions can verify
 ...
 */
type IngestPoolEventEmitter = <T extends SphereEventType>(event: T, payload: SphereEventMap[T]) => void;
/**
 ...
 */
type ProcessTokenFn = (tokenRoot: RootRef, verified: VerifiedBundle, context: ProcessTokenContext) => Promise<void>;
 ...
 */
type IngestPoolLogEmit = (level: 'debug' | 'info' | 'warn' | 'error', message: string, details?: Readonly<Record<string, unknown>>) => void;
/**
 ...
 */
interface IngestWorkerPoolOptions {
    /**
 ...
 */
declare class IngestWorkerPool {
    private readonly lru;
    private readonly perTokenMutex;
    private readonly mutexStrategy;
    private readonly processToken;
    private readonly emit;
    private readonly cidOptions;
    private readonly logEmit;
    private readonly acquireBundleFn;
    private readonly maxWorkers;
    private readonly queueCapacity;
    private readonly perTokenCap;
    private readonly bundleMaxProcessingMs;
    private readonly operatorAlertRateLimitMs;
    /**
     ...
     * trigger eviction.
     */
    private readonly operatorAlertWindows;
    /**
     ...
     * single new entry.
     */
    private readonly wakers;
    /** Promises returned by each `runWorker()` invocation. */
    private readonly workerPromises;
    /**
     ...
     *          processing.
     */
    enqueue(payload: UxfV1Payload, senderTransportPubkey: string): Promise<void>;
    /**
     ...
     */
    drainQueue(timeoutMs?: number, pollIntervalMs?: number): Promise<void>;
    ...
    private isInstantModeSoftReject;
    /**
    ...
    private incrementPerTokenCounters;
    private decrementPerTokenCounters;
 ...
 * inject in-memory fakes.
 */
interface CascadeManifestScanner {
    /**
     * Read the current manifest entry for `(addr, tokenId)`. Returns
     ...
     * recovery channel.
     */
    findChildren(addr: string, parentTokenId: string): Promise<ReadonlyArray<string>>;
}
/**
 ...
 * inject in-memory fakes.
 */
interface CascadeOutboxScanner {
    /**
     ...
     */
    findEntriesByTokenId(tokenId: string): Promise<ReadonlyArray<UxfTransferOutboxEntry>>;
}
/**
 ...
 */
type ClassifyTokenLookup = (addr: string, tokenId: string) => Promise<'coin' | 'nft' | null>;
/** Cycle / depth-overrun warning shape passed to {@link CascadeWalkerOptions.onCycleDetected}. */
interface CascadeCycleWarning {
    readonly addr: string;
 ...
 */
interface CascadeWalkerOptions {
    ...
    readonly emit: <T extends SphereEventType>(type: T, data: SphereEventMap[T]) => void;
    /**
...
declare class CascadeWalker {
     ...
     */
    cascade(addr: string, parentTokenId: string, reason: DispositionReason): Promise<CascadeResult>;
    /**
     ...
     * @internal
     */
    private _cascadeChildWithParentFlipCheck;
    /**
     ...
     * @internal
     */
    private _emitCascadeFailedForOutboxEntries;
    /** @internal */
    private _emitCycleWarning;
    /** @internal */
    private _emitScannerError;
}
 ...
 */
interface RevalidationCycleWarning {
    readonly addr: string;
    readonly parentTokenId: string;
    readonly visitedTokenId: string;
    readonly depth: number;
 ...
 */
interface RevalidationScannerError {
    readonly addr: string;
    readonly tokenId: string;
 ...
 */
interface RevalidateCascadedOptions {
    /**
 ...
 * ```
 */
declare class RevalidateCascadedRunner {
    private readonly opts;
    private readonly maxDepth;
     ...
     */
    run(addr: string, parentTokenId: string): Promise<RevalidationResult>;
    /**
     * @internal
     *
 ...
 */
type RepublishFn = (entry: UxfTransferOutboxEntry) => Promise<void>;
/**
 ...
 */
interface SendingRecoveryWorkerLogger {
 ...
 */
interface SendingRecoveryWorkerDeps {
 ...
 */
interface SendingRecoveryWorkerOptions {
 ...
 */
declare class SendingRecoveryWorker {
    private readonly deps;
    private readonly intervalMs;
    private readonly stuckThresholdMs;
    private readonly maxRetries;
    private readonly now;
    ...
    constructor(deps: SendingRecoveryWorkerDeps, options?: SendingRecoveryWorkerOptions);
    /**
     ...
     */
    private transitionToFailedTransient;
    /**
 ...
 */
type WriteSentEntryFn = (entry: UxfTransferOutboxEntry, opLabel: string) => Promise<'success' | 'failed' | 'skipped'>;
/**
 ...
 */
type OutboxWriterProvider$1 = () => Pick<OutboxWriter, 'readAllNew' | 'delete'> | null;
/**
 * Provider of the currently-installed {@link SentLedgerWriter}. Same
 * closure rationale as {@link OutboxWriterProvider}.
 */
type SentLedgerWriterProvider$1 = () => Pick<SentLedgerWriter, 'readOne'> | null;
/**
 * Logger surface — narrow on purpose so any caller-supplied logger
 * (Sphere's debug logger, a no-op stub, console) plugs in cleanly.
 * Mirrors {@link SendingRecoveryWorkerLogger}.
 */
interface SentReconciliationWorkerLogger {
    readonly warn: (message: string, context?: Record<string, unknown>) => void;
    readonly info?: (message: string, context?: Record<string, unknown>) => void;
}
/**
 ...
 */
interface SentReconciliationWorkerDeps {
    /** OUTBOX writer provider — see {@link OutboxWriterProvider}. */
    readonly outboxProvider: OutboxWriterProvider$1;
    /** SENT writer provider — see {@link SentLedgerWriterProvider}. */
    readonly sentProvider: SentLedgerWriterProvider$1;
    /**
     ...
     */
    readonly emit: <T extends SphereEventType>(type: T, data: SphereEventMap[T]) => void | Promise<void>;
 ...
 */
interface SentReconciliationWorkerOptions {
    readonly intervalMs?: number;
 ...
 */
declare class SentReconciliationWorker {
    private readonly deps;
    private readonly intervalMs;
    ...
    /** In-flight scan promise; awaited by `stop()` so callers see graceful drain. */
    private scanInFlight;
    constructor(deps: SentReconciliationWorkerDeps, options?: SentReconciliationWorkerOptions);
    /**
     ...
     */
    runScanCycle(): Promise<ReconciliationCycleResult>;
    /**
 ...
 * are all zero.
 */
interface ReconciliationCycleResult {
 ...
 */
type VerifyOutcome = 'retained' | 'missing' | 'unverifiable';
/**
 ...
 */
type VerifySentEntryFn = (entry: UxfSentLedgerEntry) => Promise<VerifyOutcome>;
/**
 ...
 * holding a dangling reference past `Sphere.destroy()`.
 */
type SentLedgerWriterProvider = () => Pick<SentLedgerWriter, 'readAll'> | null;
/**
 ...
 */
type OutboxWriterProvider = () => Pick<OutboxWriter, 'update'> | null;
/**
 * Logger surface — narrow on purpose so any caller-supplied logger
 * plugs in cleanly. Mirrors the other workers' Logger types.
 */
interface NostrPersistenceVerifierLogger {
    readonly warn: (message: string, context?: Record<string, unknown>) => void;
    readonly info?: (message: string, context?: Record<string, unknown>) => void;
}
/**
 ...
 */
interface NostrPersistenceVerifierDeps {
    /** SENT writer provider — see {@link SentLedgerWriterProvider}. */
 ...
 */
interface NostrPersistenceVerifierOptions {
    readonly intervalMs?: number;
 ...
 */
declare class NostrPersistenceVerifier {
    private readonly deps;
    private readonly intervalMs;
    private readonly verifyDelayMs;
    private readonly maxScanPerCycle;
    private readonly now;
    /** Set of SENT entry ids already classified (retained OR missing).
     *  In-memory only — restart re-arms the checks. */
    private readonly checkedIds;
    /** `true` between `start()` and the first `stop()`. */
    private running;
    /** Pending timer handle for the next scheduled cycle. */
    private timer;
     ...
     *  graceful drain. */
    private scanInFlight;
    constructor(deps: NostrPersistenceVerifierDeps, options?: NostrPersistenceVerifierOptions);
    /**
     ...
     */
    runScanCycle(): Promise<VerifierCycleResult>;
    /**
     ...
     */
    private attemptRetentionRepublish;
    private emitRepublishSkipped;
    private emitRetentionWarning;
    /**
 ...
 */
type SentLookupProvider = () => Pick<SentLedgerWriter, 'contains'> | null;
/**
 ...
 */
type OutboxLookupProvider = () => Pick<OutboxWriter, 'readAll'> | null;
/**
 ...
 */
type TokenIterableProvider = () => Iterable<Token>;
/**
 ...
 * those tokens.
 */
type ExtractCurrentStateHashFn = (token: Token) => string;
/**
 ...
 */
type TransitionToAuditFn = (params: {
 ...
 * plugs in cleanly. Mirrors the other workers' Logger types.
 */
interface SpentStateRescanWorkerLogger {
    readonly warn: (message: string, context?: Record<string, unknown>) => void;
    readonly info?: (message: string, context?: Record<string, unknown>) => void;
}
/**
 ...
 */
interface SpentStateRescanWorkerDeps {
 ...
 */
interface SpentStateRescanWorkerOptions {
    readonly intervalMs?: number;
 ...
 * for full semantics.
 */
declare class SpentStateRescanWorker {
    private readonly deps;
    private readonly intervalMs;
    private readonly perTokenIntervalMs;
    private readonly maxConcurrent;
    private readonly throwBackoffThreshold;
    private readonly throwBackoffMs;
    private readonly now;
     ...
     *  graceful drain. */
    private scanInFlight;
    constructor(deps: SpentStateRescanWorkerDeps, options?: SpentStateRescanWorkerOptions);
    /**
     * Start the periodic scan. Idempotent. The first scan fires after
     * one `intervalMs` delay so the worker doesn't race with whatever
     * just instantiated it.
     ...
     * without waiting for the timer.
     */
    runScanCycle(): Promise<SpentRescanCycleResult>;
    /**
     ...
     */
    private computeOutboxActiveTokenIds;
    /**
     ...
     */
    private computeSuspectedSiblingInstance;
    private recordThrow;
 ...
 * are all zero.
 */
interface SpentRescanCycleResult {
...
interface PoolWriteAdapter {
     ...
     */
    isProofAttached(tokenId: string, queueEntryRequestId: string): Promise<boolean>;
    /**
     ...
     */
    attachProof(tokenId: string, queueEntryRequestId: string, proof: InclusionProof): Promise<void>;
}
/**
 ...
 */
interface TombstoneWriteAdapter {
    /**
 ...
 */
interface FinalizationQueueAdapter {
    /**
 ...
 * injection.
 */
interface ManifestCidRewriteContext {
 ...
 * deterministic fakes.
 */
interface TrustBaseStalenessOptions {
    /**
 ...
 */
declare class TrustBaseStaleness {
     ...
     */
    isTrustBaseStale(aggregatorId?: string): boolean;
    /**
     ...
     */
    recordNotAuthenticated(aggregatorId?: string, priorRefreshTag?: number | null): NotAuthenticatedRecord;
    /**
     ...
     * @param aggregatorId — defaults to `'default'`.
     */
    recordAuthenticatedOk(aggregatorId?: string): void;
    /**
     ...
     * @param aggregatorId — defaults to `'default'`.
     */
    getRefreshTag(aggregatorId?: string): number;
    /**
     ...
     */
    refreshTrustBase(aggregatorId?: string, signal?: AbortSignal): Promise<RefreshOutcome>;
    /**
     ...
     * @param aggregatorId — defaults to `'default'`.
     */
    lastNotAuthenticatedAt(aggregatorId?: string): number | null;
    /**
     ...
     * @param aggregatorId — defaults to `'default'`.
     */
    lastRefreshAt(aggregatorId?: string): number | null;
    /**
 ...
 */
type SubmitOutcomeKind = 'SUCCESS' | 'REQUEST_ID_EXISTS' | 'AUTHENTICATOR_VERIFICATION_FAILED' | 'REQUEST_ID_MISMATCH' | 'TRANSIENT';
/**
 ...
 */
interface AnchoredProofDescriptor {
    /**
...
type PollOutcome = {
    ...
    readonly kind: 'PATH_NOT_INCLUDED' | 'PATH_INVALID' | 'NOT_AUTHENTICATED' | 'TRANSIENT';
    readonly error?: string;
};
/**
...
interface RequestContext {
     ...
     */
    readonly nextEntryRest: Pick<NonNullable<ManifestCidRewriteContext['nextEntryRest']>, 'status' | 'conflictingHeads' | 'invalidReason'>;
}
/**
 ...
 */
interface FinalizationAggregatorClient {
    /**
...
interface PoolReadAdapter {
     ...
     */
    getAttachedProof(tokenId: string, requestId: string): Promise<AnchoredProofDescriptor | null>;
}
 ...
 */
interface FinalizationOutboxWriter {
    /**
 ...
 */
interface FinalizationWorkerCaps {
 ...
 * or OrbitDB store.
 */
interface FinalizationWorkerSenderOptions {
 ...
 */
declare class FinalizationWorkerSender {
    private readonly options;
    private readonly perAggregatorSemaphore;
    private readonly perAggregator;
    private readonly perToken;
    private readonly maxSubmitRetries;
    private readonly maxProofErrorRetries;
    private readonly pollingWindowMs;
    private readonly perTokenMutexStrategy;
    private readonly aggregatorId;
    private readonly trustBaseStaleness;
    private readonly scanIntervalMs;
    private readonly maxEntriesPerScan;
    private readonly manualScan;
    private running;
     ...
     */
    private scanReadFailureEmittedAtStreak;
    /**
     ...
     */
    private scanOversizeEmittedAtStreak;
    /**
     ...
     */
    processOne(entry: UxfTransferOutboxEntry): Promise<ProcessOneResult>;
 ...
 */
interface HydratedTx extends TxLike {
 ...
 */
type HydrateChainFn = (tokenRootHash: ContentHash, pool: ReadonlyMap<ContentHash, UxfElement>) => Promise<HydratedChain>;
/**
 ...
 */
type ReadLocalManifestFn = (tokenId: string) => Promise<ManifestEntryDelta | undefined>;
/**
 ...
 */
type EvaluatePredicateFn = (predicate: unknown, ourPubkey: Uint8Array) => Promise<EvaluatePredicateResult>;
type OracleIsSpentFn = (stateHash: string) => Promise<boolean>;
/**
 ...
 *    proof.
 */
interface DispositionRevaluateInput {
    readonly tokenRootHash: ContentHash;
    readonly pool: ReadonlyMap<ContentHash, UxfElement>;
 ...
 * recorders.
 */
interface FinalizationDispositionWriter {
    write(addr: string, record: DispositionRecord): Promise<void>;
}
/**
 ...
 */
interface RevaluateHooksProvider {
    /**
     ...
     */
    buildRevaluateInput(addr: string, tokenId: string): Promise<DispositionRevaluateInput | null>;
}
/**
 ...
 */
interface FinalizationWorkerRecipientCaps {
    /** §6.1 — at most this many in-flight aggregator calls per
     *  endpoint. Default {@link MAX_CONCURRENT_POLLS_PER_AGGREGATOR}. */
    readonly perAggregator?: number;
    /** §6.1 — at most this many in-flight polls for the same tokenId.
 ...
 */
interface FinalizationWorkerRecipientOptions {
    /** Address id this worker scopes to. */
    readonly addressId: string;
 ...
 */
interface ProcessQueueEntryResult {
    readonly entryId: string;
    readonly tokenId: string;
    readonly outcome: {
 ...
 */
interface ProcessOneTokenResult {
    readonly tokenId: string;
    ...
    readonly terminal: 'valid' | 'unspendable' | 'not-our-state' | 'conflicting' | 'invalid' | 'in-progress';
}
/**
 ...
 * independent.
 */
declare class FinalizationWorkerRecipient {
    private readonly options;
    private readonly perAggregatorSemaphore;
    private readonly perAggregator;
    private readonly perToken;
    ...
    private readonly trustBaseStaleness;
    private readonly scanIntervalMs;
    private readonly maxTokensPerScan;
    private readonly manualScan;
    private running;
    private stopRequested;
    private loopPromise;
    ...
    private readonly inFlightTokens;
    /**
     ...
     * @internal
     */
    private readonly cascadeTombstones;
    /**
     ...
     * @internal
     */
    private cascadeTombstoneHighWatermarkAlerted;
    /**
     ...
     * @internal
     */
    private cascadeTombstoneLastEvictionAlertAt;
    /**
     ...
     */
    processOneToken(tokenId: string): Promise<ProcessOneTokenResult>;
     ...
     */
    processQueueEntry(entry: FinalizationQueueEntry): Promise<ProcessQueueEntryResult>;
    /**
     ...
     * @internal
     */
    private insertCascadeTombstone;
    private applyHardFailCascade;
    /**
     ...
     * @internal
     */
    private emitTransferIncomingConfirmed;
    /**
 ...
 */
interface OrphanSpendingFinding {
    /** The orphan token id. */
    readonly tokenId: string;
 ...
 */
interface LegacyShapeAdapterRunner {
    /**
     ...
     */
    processLegacy(payload: unknown, senderTransportPubkey: string): Promise<void>;
}
...
declare class PaymentsModule {
     ...
     */
    private legacyShapeAdapterRunner;
    ...
    private static readonly PROOF_POLLING_MAX_CUMULATIVE_ATTEMPTS;
     ...
     */
    private writeSentEntryFromOutbox;
    /**
     ...
     */
    private assertNoDuplicateBundleMembership;
    /**
     ...
     * at boot).
     */
    detectOrphanSpendingTokens(): Promise<OrphanSweepResult>;
    /**
     ...
     * Issue #174.
     */
    private _spentStateRescanTransitionToAudit;
    /**
     ...
     */
    private finalizationWorkerRecipient;
    /**
     ...
     */
    private _workerAbortController;
    /**
     ...
     * @internal
     */
    private readonly _senderOutboxMap;
    /**
     ...
     * @internal
     */
    private _dispatcherInFlightCount;
    /**
     ...
     * @internal
     */
    private readonly _senderRequestContextMap;
    /**
     ...
     * @internal
     */
    private readonly _recipientRequestContextMap;
    /**
     ...
     * @internal
     */
    private readonly _recipientFinalizationContext;
    /**
     ...
     * @internal
     */
    private _recipientFinalizationQueue;
    /**
     ...
     * @internal
     */
    private _recipientSaveFailureStreakClear;
    /**
     ...
     * @internal
     */
    private _recipientContextHydrationPromise;
    /**
    ...
    configureRecipientPersistedStorage(opts: {
        readonly finalizationQueueStorage?: FinalizationQueueStorage;
        readonly recipientContextStorage?: OrbitDbRecipientContextStorageAdapter | InMemoryRecipientContextStorageAdapter;
    }): void;
    /**
     ...
     */
    configureOperatorEscapeHatchStorage(dispositionStorage: DispositionPerEntryStorage, options?: {
        readonly verifyProof?: ProofVerifier;
        readonly graftCallback?: ImportProofGraftCallback;
        readonly overrideCallback?: ImportProofOverrideCallback;
    }): void;
     ...
     */
    setSpentStateRescanTransitionToAudit(transition: TransitionToAuditFn | null): void;
    /**
     ...
     */
    importInclusionProof(addr: string, tokenId: string, proof: ImportableInclusionProof, options?: ImportInclusionProofCallOptions): Promise<ImportProofResult>;
    /**
     ...
     */
    revalidateCascadedChildren(addr: string, parentTokenId: string): Promise<RevalidationResult>;
    /**
     ...
     */
    send(originalRequest: TransferRequest, internal?: {
     ...
     */
    private extractCoinAmountForCache;
    /**
     ...
     */
    sendInstant(originalRequest: TransferRequest, options?: InstantSplitOptions): Promise<InstantSplitResult>;
    /**
     ...
     */
    private drainPendingFinalizations;
    /**
     ...
     */
    importTokens(txfTokens: readonly TxfToken[], options?: {
     ...
     */
    private isReceivedLegacyPending;
    /**
     ...
     */
    private primeProxyAddressCache;
    /**
     ...
     */
    private latestStatePredicateMatchesWallet;
    /**
     ...
     */
    private recoverStrandedReceivedTokens;
    /**
     ...
     * finalized SDK token.
     */
    private finalizeStrandedReceivedToken;
    /**
     ...
     */
    private tryLocalFinalizeUnconfirmed;
    private resolveLegacyReceivedToken;
    /**
     ...
     */
    private resolveLegacyReceivedTokenViaGetProof;
    /**
     ...
     */
    private _lastPinnedV5Json;
    private _lastPinnedV5Ref;
     ...
     */
    clearNametagByName(name: string): Promise<boolean>;
    /**
     ...
     */
    private computeOutboundAssetKinds;
    /**
     ...
     */
    private resolveOutboundWireProtocol;
    /**
     ...
     */
    private maybeEmitCapabilityWarning;
    /**
     ...
     */
    private recordUxfBundleSentHistory;
    /**
     ...
     */
    private dispatchUxfConservativeSend;
    /**
     ...
     */
    private dispatchUxfInstantSend;
    /**
     ...
     */
    private validateSourceOwnership;
    /**
     ...
     */
    private submitCommitmentClassified;
    /**
     ...
     */
    private emitDoubleSpendDetectedIfApplicable;
     ...
     */
    private deriveRecipientAddressFor;
    /**
     ...
     */
    private resolveExpectedTransactionAddress;
    /**
     ...
     */
    private tryRecoverSigningServiceForRecipient;
    /**
     ...
     * `since` filter.
     */
    private awaitAllProvidersDurable;
    /**
     ...
     */
    private _lastPinnedOutboxJson;
    private _lastPinnedOutboxRef;
    /**
     ...
     */
    private _outboxChain;
    private enqueueOutboxOp;
...
interface SphereEventMap {
    ...
    'transfer:operator-alert': {
        readonly code: DispositionReason;
        readonly tokenId?: string;
        readonly bundleCid?: string;
        readonly observedTokenContentHash?: ContentHash;
        readonly senderTransportPubkey?: string;
    ...
    'transfer:retention-republish-skipped': {
        ...
        readonly nostrEventId: string;
        readonly bundleCid: string;
        readonly reason: 'no-outbox-writer' | 'entry-tombstoned-or-missing' | 'wrong-status' | 'transition-failed';
 ...
 */
type ProfileErrorCode = 'PROFILE_NOT_INITIALIZED' | 'ORBITDB_WRITE_FAILED' | 'ORBITDB_READ_FAILED' | 'ORBITDB_CONNECTION_FAILED' | 'ORBITDB_NOT_INSTALLED' | 'BUNDLE_NOT_FOUND' | 'CONSOLIDATION_IN_PROGRESS' | 'MIGRATION_FAILED' | 'ENCRYPTION_FAILED' | 'DECRYPTION_FAILED' | 'BOOTSTRAP_REQUIRED'
...
type OriginTag = 'user' | 'system' | 'replicated';
...
declare const OPLOG_ENTRY_SCHEMA_VERSION: 1;
/**
 ...
 */
declare const OPLOG_ENTRY_LEGACY_VERSION: 0;
type OpLogEntrySchemaVersion = typeof OPLOG_ENTRY_SCHEMA_VERSION;
interface OpLogEntryEnvelope {
    /**
     * Schema version.
 ...
 * is not used.
 */
declare class OrbitDbAdapter implements ProfileDatabase {
    /** The Helia IPFS node. */
    private helia;
     ...
     * coherence check.
     */
    putEntry(key: string, entry: OpLogEntryEnvelope): Promise<void>;
    /**
 ...
 */
interface ConsolidationPendingState {
 ...
 */
type MigrationPhase = 'syncing' | 'transforming' | 'persisting' | 'verifying' | 'cleaning' | 'complete';
/**
...
declare const DEFAULT_ENCRYPTION_CONFIG: ProfileEncryptionConfig;
/**
...
type SyncEventType = 'sync:started' | 'sync:completed' | 'sync:failed' | 'sync:remote-updated' | 'sync:bundle-added' | 'sync:bundle-removed';
 ...
 */
type ProfileKeyMap = Readonly<Record<string, ProfileKeyMapEntry>>;
/**
 ...
 */
declare const IPFS_STATE_KEYS_PATTERN: RegExp;
/**
 ...
 */
declare function computeAddressId(directAddress: string): string;
...
declare const PROFILE_HKDF_INFO = "uxf-profile-encryption";
declare function deriveProfileEncryptionKey(masterKey: Uint8Array): Uint8Array;
/**
 ...
 */
declare function encryptProfileValue(key: Uint8Array, plaintext: Uint8Array, aad?: Uint8Array): Promise<Uint8Array>;
/**
 ...
 *         or malformed input
 */
declare function decryptProfileValue(key: Uint8Array, encrypted: Uint8Array, aad?: Uint8Array): Promise<Uint8Array>;
/**
 ...
 */
declare function encryptString(key: Uint8Array, value: string, aad?: Uint8Array): Promise<Uint8Array>;
/**
 ...
 */
declare function decryptString(key: Uint8Array, encrypted: Uint8Array, aad?: Uint8Array): Promise<string>;
 ...
 */
interface NostrReplicationConfig {
 ...
 * ```
 */
declare class NostrReplicationBridge {
    #private;
    private readonly config;
    static readonly SEEN_EVENT_CACHE_MAX = 10000;
    private ws;
    private pubkey;
    private running;
    private subscriptionId;
    private sequence;
    constructor(config: NostrReplicationConfig);
    /**
     ...
     */
    publishEntry(entryData: Uint8Array): Promise<void>;
    /**
     ...
     */
    fetchMissedEntries(since: number): Promise<Uint8Array[]>;
 ...
 */
declare const DEFAULT_LIST_KEYS_MAX_RESULTS = 1024;
/**
 ...
 */
declare class InMemoryDispositionStorageAdapter implements DispositionPerEntryStorage {
 ...
 */
declare function pinToIpfs(gateways: string[], data: Uint8Array, timeoutMs?: number): Promise<string>;
/**
 ...
 */
declare function fetchFromIpfs(gateways: string[], cid: string, timeoutMs?: number, maxSizeBytes?: number, helia?: unknown): Promise<Uint8Array>;
/**
 ...
 */
declare function fetchCarFromIpfs(gateways: string[], rootCid: string, timeoutMs?: number, maxSizeBytesPerBlock?: number, helia?: unknown): Promise<Uint8Array>;
/**
 ...
 */
declare function verifyCidAccessible(gateways: string[], cid: string, timeoutMs?: number): Promise<boolean>;
...
declare class ConsolidationEngine {
    ...
    constructor(db: ProfileDatabase, encryptionKey: Uint8Array, ipfsGateways: string[], retentionMs?: number);
    /**
     ...
     */
    isConsolidationInProgress(): Promise<boolean>;
    /**
     ...
     */
    private readPendingState;
    /**
     * Write the consolidation pending state to OrbitDB.
     */
    private writePendingState;
    /**
 ...
 * ```
 */
declare class ProfileMigration {
     ...
     */
    needsMigration(legacyStorage: StorageProvider): Promise<boolean>;
    /**
     ...
     */
    getMigrationPhase(legacyStorage: StorageProvider): Promise<MigrationPhase | null>;
    /**
     ...
     */
    migrate(legacyStorage: StorageProvider, legacyTokenStorage: TokenStorageProvider<TxfStorageDataBase>, profileStorage: ProfileStorageProvider, profileTokenStorage: ProfileTokenStorageProvider): Promise<MigrationResult>;
    /**
...
interface LegacyImportResult {
     ...
     */
    readonly skippedByCode: Readonly<Record<'duplicate' | 'tombstoned' | 'genesis-exists' | 'unknown', number>>;
 ...
 * for dedup.
 */
declare function deriveTombstonesFromArchived(data: TxfStorageDataBase, now?: number): TxfTombstone[];
/**
 ...
 */
declare function deriveSentFromArchived(data: TxfStorageDataBase, now?: number): TxfSentEntry[];
/**
 ...
 */
declare function deriveHistoryFromArchived(data: TxfStorageDataBase, ourAddress: string | undefined, now?: number): HistoryRecord[];

/**
 * Result of creating Profile-backed providers.
 */
interface ProfileProviders {
 ...
 */
interface BrowserProfileProvidersConfig {
 ...
 */
interface BrowserProfileProviders {
