...
async function extractCarRootCid(carBytes) {
  ...
  if (roots === void 0) {
    let reader;
    try {
      reader = await import_car.CarReader.fromBytes(carBytes);
    } catch (cause) {
      throw new SphereError(
...
var import_PredicateEngineService = require("@unicitylabs/state-transition-sdk/lib/predicate/PredicateEngineService");
...
var import_RequestId3 = require("@unicitylabs/state-transition-sdk/lib/api/RequestId");
var import_DataHash3 = require("@unicitylabs/state-transition-sdk/lib/hash/DataHash");
var UnicityAggregatorProvider = class _UnicityAggregatorProvider {
  ...
  async getProof(requestId) {
    ...
    try {
      ...
      try {
        import_InclusionProof2.InclusionProof.fromJSON(proof);
      } catch (parseErr) {
        logger.warn(
          "Aggregator",
  ...
  async validateToken(tokenData) {
    ...
    let sdkToken = null;
    try {
      sdkToken = await import_Token.Token.fromJSON(tokenData);
    } catch {
    }
    try {
  ...
  async derivePredicatePublicKeyHex(sdkToken) {
    ...
    let predicate;
    try {
      predicate = await import_PredicateEngineService.PredicateEngineService.createPredicate(statePredicate);
    } catch {
      return null;
    }
  ...
  async verifyInclusionProof(input) {
    ...
    let result = false;
    try {
      const proof = import_InclusionProof2.InclusionProof.fromJSON(input.proofJson);
      if (!proof.authenticator) {
        ...
        if (proofTxHashHex === null) {
          ...
          const requestId = await import_RequestId3.RequestId.create(
            proof.authenticator.publicKey,
            proof.authenticator.stateHash
          );
          const status = await proof.verify(this.trustBase, requestId);
          result = status === import_InclusionProof2.InclusionProofVerificationStatus.OK;
        }
      }
    } catch (err) {
  ...
  async isSpent(publicKey, stateHash) {
    ...
    try {
      ...
      const stateHashDataHash = import_DataHash3.DataHash.fromJSON(stateHash);
      const requestId = await import_RequestId3.RequestId.create(pubkeyBytes, stateHashDataHash);
