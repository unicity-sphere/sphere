...
import { NFT_DOCUMENT_MEDIA_TYPE } from '@unicitylabs/sphere-sdk';
import type { CoinlessToken, NftLink, NftMedia, NftMetadata } from '@unicitylabs/sphere-sdk';
...
const DOCUMENT_LINK: NftLink = {
  kind: 'link',
  media_type: NFT_DOCUMENT_MEDIA_TYPE,
  uri: 'https://example.com/cat.cbor',
  sha256: 'ee'.repeat(32),
};
...
describe('nftTitle', () => {
  it("prefers the NFT's own metadata name over the registry's class name", () => {
    expect(nftTitle(token({ name: 'Cats' }), metadata())).toBe('Cool Cat #7');
  });
  ...
  it('falls back to the registry name when there is no reading, or the NFT is a bare file', () => {
    ...
    expect(nftTitle(token({ name: 'Cats' }), null)).toBe('Cats');
    expect(nftTitle(token({ name: 'Cats' }), MEDIA)).toBe('Cats');
    expect(nftTitle(token({ name: 'Cats' }), LINK)).toBe('Cats');
  });

  it('takes no name from an unresolved metadata document link — the name is in the document', () => {
    expect(nftTitle(token({ name: 'Cats' }), DOCUMENT_LINK)).toBe('Cats');
  });
...
describe('nftThumbnailRef', () => {
  it("previews metadata's image", () => {
    expect(nftThumbnailRef(metadata({ image: LINK }))).toBe(LINK);
    expect(nftThumbnailRef(metadata({ image: MEDIA }))).toBe(MEDIA);
  });

  it('previews nothing for metadata without an image — animation_url is not a thumbnail', () => {
    expect(nftThumbnailRef(metadata({ animation_url: MEDIA }))).toBeNull();
  });

  it('previews a bare media or link NFT as itself', () => {
    expect(nftThumbnailRef(MEDIA)).toBe(MEDIA);
    expect(nftThumbnailRef(LINK)).toBe(LINK);
  });

  it('previews nothing without a reading', () => {
    expect(nftThumbnailRef(undefined)).toBeNull();
    expect(nftThumbnailRef(null)).toBeNull();
  });
});
