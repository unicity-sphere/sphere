...
export function NftMediaView({ media, alt, variant }: NftMediaViewProps) {
  ...
  const onError = () => setFailedUrl(playable);

  if (variant === 'thumb') {
    return playable ? (
      <img src={playable} alt={alt} onError={onError} className="w-full h-full object-cover" />
    ) : (
  ...
  if (playable && kind === 'image') {
    body = <img src={playable} alt={alt} onError={onError} className="max-h-80 w-full object-contain" />;
  } else if (playable && kind === 'video') {
    body = (
      <video src={playable} controls playsInline aria-label={alt} onError={onError} className="max-h-80 w-full" />
    );
  } else if (playable && kind === 'audio') {
    body = <audio src={playable} controls aria-label={alt} onError={onError} className="w-full p-2" />;
