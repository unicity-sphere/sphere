import { Wallet, Boxes, Plug, BarChart3, TrendingUp, Clock, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <StatCard
          label="Balance"
          value={`${formatNumber(config.balance)} tokens`}
          sublabel={`Cap: ${config.maxTokensPerTask} per task`}
          Icon={Wallet}
          tone="orange"
        ...
        <StatCard
          label="Total spent"
          value={`${formatNumber(stats.totalTokens)} tokens`}
          sublabel={lastTaskRelative}
          Icon={TrendingUp}
          tone="emerald"
        ...
        {stats.lastTaskAt === null ? (
          <div className="flex flex-col items-center text-center py-6">
            ...
            <p className="text-xs text-neutral-500 dark:text-white/45">
              Start chatting with {config.name} to see activity here.
            </p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {Object.entries(stats.byTaskType)
              ...
              .map(([type, s]) => (
                <div
                  key={type}
                  className="flex items-center justify-between py-2 px-3 rounded-lg bg-white dark:bg-white/4"
                >
                  <span className="text-xs text-neutral-700 dark:text-white/75 capitalize">
                    {type.replace('-', ' ')}
                  </span>
                  <span className="text-xs font-medium text-neutral-900 dark:text-white">
                    {s.count} task{s.count === 1 ? '' : 's'} · {s.tokens} tokens
                  </span>
                </div>
              ))}
            <button
